...
def main():
    ...
    try:
        ...
        # For batch file integration, also output in a parseable format
        print("=== FOR BATCH INTEGRATION ===")
        conda_versions = []
        for conda_config in result['conda_configs']:
            conda_versions.append(f"{conda_config['name']}:{conda_config['version']}")

        print(f"GZ_COLLECTION={result['collection']}")
        print(f"GZ_CONDA_CONFIGS={';'.join(conda_versions)}")
