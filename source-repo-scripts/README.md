...
DRY_RUN=true [bump_dependency.bash](bump_dependency.bash) <collection> <library>;<library> <version>;<version> <issue_number> <prev_collection> [<docs_branch>]

# real execution
[bump_dependency.bash](bump_dependency.bash) <collection> <library>;<library> <version>;<version> <issue_number> <prev_collection> [<docs_branch>]
```
...
```bash
./[bump_dependency.bash](bump_dependency.bash) fortress "gz-rendering;sdformat" "6;12" 428 edifice
"chapulina/fortress"
```

### [merge_forward_pull_request.bash](merge_forward_pull_request.bash)
...
Requires the 'gh' CLI to be installed.
```bash
./[merge_forward_pull_request.bash](merge_forward_pull_request.bash) <from_branch> <to_branch>
```
...
git merge --continue
/path/to/[merge_forward_pull_request.bash](merge_forward_pull_request.bash) ign-rendering6 main
```

### [source_changelog.bash](source_changelog.bash)
...
```
[source_changelog.bash](source_changelog.bash) <last_version>
```
...
bash [release_summary.bash](release_summary.bash) <prev release version> <new release version>
```
