...
do_install() {
	cat >&2 <<-'EOF_INIT'
	=======================================================
	GZ INSTALLATION SCRIPT
	=======================================================

	This script is installing the latest stable version of
	Gz Simulator available from your package manager

	EOF_INIT

	if command_exists gz; then
		cat >&2 <<-'EOF'
		Warning: the "gz" command appears to already exist on this system.

		If you already have gz installed, this script can cause trouble, which is
		why we're displaying this warning and provide the opportunity to cancel the
		installation.

		If you installed the current gz package using this script and are using it
		again to update gz, you can safely ignore this message.
