...
for pkg in ${PKGS}; do
	...
	# Modify package name
	sed -i -e "s/Package: @(Package)/Package: @(Package.replace('gazebo-','gazebo7-'))/" debian/control.em
	git commit debian/control.em -m "Patch name to release -7 version"
	# Include conflict with same package (not current)
	sed -i -e "/^Depends/aConflicts: @(Package), @(Package)-current, @(Package.replace('gazebo-','gazebo3-')), @(Package.replace('gazebo-','gazebo4-')), @(Package.replace('gazebo-','gazebo5-')), @(Package.replace('gazebo-','gazebo6-'))" debian/control.em
