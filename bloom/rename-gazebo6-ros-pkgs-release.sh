...
#
#   - Rename the Package name to $(Package)6
...
for pkg in ${PKGS}; do
	...
	# no ros_control on jade
	if ${distro} == 'jade' && ${pkg} == 'gazebo_ros_control'; then
	    continue
	fi
	echo " - Processing $pkg in $distro"
        git checkout debian/$distro/$pkg
	# Modify package name
	sed -i -e "s/Package: @(Package)/Package: @(Package.replace('gazebo-','gazebo6-'))/" debian/control.em
	git commit debian/control.em -m "Patch name to release -6 version"
	# Include conflict with same package (not current)
	sed -i -e "/^Depends/aConflicts: @(Package), @(Package)-current, @(Package.replace('gazebo-','gazebo4-')), @(Package.replace('gazebo-','gazebo5-'))" debian/control.em
